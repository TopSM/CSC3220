...
class SellerRow extends Component {
  ...
  render() {
    const ActionButtons = () =>
      this.props.seller.account_approved ? (
        <Button onClick={this.handleRejectSeller}>Reject</Button>
      ) : (
        <Button onClick={this.handleApproveSeller} positive>
          Approve
        </Button>
      );
    ...
    return (
      <Table.Row error={!this.props.seller.account_approved}>
        ...
        <Table.Cell>
          {this.props.seller.name.first} {this.props.seller.name.last}
        </Table.Cell>
